...
// Generated: 5/10/2017 2:17:22 PM by kwhite
...
declare namespace Isc {
	...
	export interface AnimateShowEffect  {
		...
		/** (Read only) For hide animations of type "wipe and "slide" this attribute specifies where the wipe / slide should finish. Valid options are "T" (vertical animation upwards to the top of the canvas, the default behavior) and "L" (horizontal animation to the left of the canvas).
		 * Flags: IR */
		 endsAt?: string; // Flags=IR
		/** (Read only) For show animations of type "wipe" and "slide" this attribute specifies where the wipe / slide should originate. Valid values are "T" (vertical animation from the top down, the default behavior), and "L" (horizontal animation from the left side).
		 * Flags: IR */
	...
	export interface Hilite  {
		...
		/** (Read only) Can highlight be edited from header context menu? Setting attribute to false prevents editing. A null or true value allows editing.
		...
		/** (Read only) CSS text to be applied to cells where this hilite is applied, for example, "background-color:#FF0000"
		 * Flags: IR, Group: hiliting */
		...
		/** (Read only) Name of the field, or array of fieldNames, this hilite should be applied to. If unset, hilite is applied to every field of the record.
		 * Flags: IR, Group: hiliting */
		...
		/** (Read only) URL of an icon to show when this hilite is applied to a cell. Position of the icon is controlled by DataBoundComponent.hiliteIconPosition or ListGridField.hiliteIconPosition.
		 * Flags: IR, Group: hiliting */
		 icon?: string /* SCImgURL */; // Flags=IR
		/** (Read only) Unique id for this hilite definition. For hilites that include Hilite.criteria this is not required. If you are explicitly marking records for hiliting, set DataBoundComponent.hiliteProperty on the record to this id.
		 * Flags: IR, Group: hiliting */
		 id?: string /* String */; // Flags=IR
		/** (Read only) HTML which replaces the cell's textual value where this hilite is applied. Note that sorting, filtering, etc behavior will still operate on the underlying value. For example, if there is a date field with the FilterEditor enabled, the default search interface will still offer date-range based filtering even if hilites have caused values to be displayed as text such as "current" or "past due".
		 * Flags: IR, Group: hiliting */
	...
	export interface EditNode  extends PaletteNode {
		...
		/** (Read only) Properties to be applied to the EditNode.liveObject','liveObject.Canvas.editProxy','editProxy when created. Note that the editProxy is created the first time a component is placed into editMode, so any editProxyProperties must be set before then.
		 * Flags: IR */
		 editProxyProperties?: EditProxyProps /* EditProxy Properties */; // Flags=IR
		/** (Read only) Live version of the object created from the EditNode.defaults. For example, if EditNode.type is "ListGrid", liveObject would be a ListGrid.
		 * Flags: IR */
		 liveObject?: any /* Object */; // Flags=IR
		/** (Read only) SCClassName of the EditNode.liveObject , for example, "ListGrid".
		 * Flags: IR */
		 type?: string /* SCClassName */; // Flags=IR
		/** (Read only) Shortcut property to be applied to the EditNode.liveObject','liveObject.Canvas.editProxy','editProxy when created.
		 * Flags: IR */
	...
	export interface SectionStackSection  {
		/** (Read only) This attribute controls whether or not the expand/collapse UI control is shown on the header of this section. Any section can still be expanded/collapsed programmatically, regardless of this setting.
		 * Flags: I */
		...
		/** (Read only) If set to false, then this sectionHeader will not be able to be dragged to perform a drag reorder, if SectionStack.canReorderSections is true. You can also disable dropping other sections before this one by setting Canvas.canDropBefore','canDropBefore to false.
		 * Flags: I */
		 canReorder?: boolean; // Flags=I
		/** (Read only) If true, the header for this Section will be included in the page's tab order for accessibility. May also be set at the SectionStack level via SectionStack.canTabToHeaders. See accessibility.
		 * Flags: IR */
		 canTabToHeader?: boolean; // Flags=IR
		/** (Read only) If the title for this section header is too large for the available space, should the title be clipped? This feature is supported only in browsers that support the CSS UI text-overflow property (IE6+, Firefox 7+, Safari, Chrome, Opera 9+).
		 * Flags: IR */
		 clipTitle?: boolean /* Boolean */; // Flags=IR
		/** (Read only) Custom controls to be shown on top of this section header. These controls are shown in the SectionHeader.controlsLayout. Note that this is an init-time property. If you need to dynamically change what controls are displayed to the user, we would recommend embedding the controls in a Layout or similar container. This will allow you to show/hide or add/remove members at runtime by manipulating the existing control(s) set up at init time.
		 * Flags: IR */
		 controls?: Array<Canvas> /* Array of Canvas */; // Flags=IR
		/** (Read only) Sections default to the collapsed state unless SectionStackSection.showHeader is set to false in which case they default to the expanded state. This attribute allows you to explicitly control the expand/collapse state of the section by overriding the above default behavior.
		 * Flags: I */
		 expanded?: boolean; // Flags=I
		/** (Read only) Sections default to the visible state. This attribute allows you to explicitly control the visible/hidden state of the section by overriding the above default behavior.
		 * Flags: I */
		 hidden?: boolean; // Flags=I
		/** (Read only) Base filename of the icon that represents open and closed states. The default settings also change the icon for disabled sections, so a total of four images are required (opened, closed, Disabled_opened, Disabled_closed). Not shown if SectionStackSection.canCollapse is false.
		 * Flags: IR */
		 icon?: string /* SCImgURL */; // Flags=IR
		/** (Read only) Optional ID for the section. If SectionStack.useGlobalSectionIDs is true, this property will be applied to the generated SectionStackHeader widget as a standard widget ID, meaning it should be unique within a page. Backcompat Note: Section stack sections may be uniquely identified within a stack via the SectionStackSection.name attribute (introduced in Jan 2010). Prior to this, the section ID attribute was used in this way (and would not be applied to the section header as a widget ID). For backwards compatibility this is still supported: If section.name is unspecified for a section but section.ID is set, the ID will be used as a default name attribute for the section. For backwards compatibility we also disable the standard behavior of having the section.ID being applied to the generated section header (thereby avoiding the page-level uniqueness requirement) by defaulting SectionStack.useGlobalSectionIDs to false.
		 * Flags: IR */
		 ID?: string /* String */; // Flags=IR
		/** (Read only) List of Canvases that constitute the section. These Canvases will be shown and hidden together.
		 * Flags: I */
		 items?: Array<Canvas> /* Array of Canvas */; // Flags=I
		/** (Read only) Identifier for the section. This can be used later in calls to SectionStack APIs such as SectionStack.expandSection and SectionStack.collapseSection. Note that if no name is specified for the section, one will be auto-generated when the section is created. This property should be a string which may be used as a valid JavaScript identifier (should start with a letter and not contain space or special characters such as "*").
		 * Flags: IR */
		 name?: string /* String */; // Flags=IR
		/** (Read only) If set to false, then the items in this section will not be resized by sectionHeader repositioning. You may also set this flag directly on any of the items in any section to cause that item to not be resizeable.
		 * Flags: I */
	...
	export interface MenuItem  extends ListGridRecord {
		/** (Read only) Whether a click on this specific menuItem automatically dismisses the menu. See Menu.autoDismiss.
		 * Flags: IR, Group: menuBasic */
		 autoDismiss?: boolean /* Boolean */; // Flags=IR
		/** (Read only) A MenuItem that has a submenu normally cannot be selected, instead clicking or hitting Enter while keyboard focus is on the item shows the submenu. Setting canSelectParent:true allows a menu item with a submenu to be selected directly.
		 * Flags: IR */
		 canSelectParent?: boolean; // Flags=IR
		/** (Read only) If true, this item displays a standard checkmark image to the left of its title. You can set the checkmark image URL by setting Menu.checkmarkImage. If you need to set this state dynamically, use MenuItem.checkIf instead.
		 * Flags: IR, Group: menuIcons */
		 checked?: boolean; // Flags=IR
		/** (Read only) The filename for this item's custom icon when the item is disabled. If both this property and MenuItem.checked are both specified, only the icon specified by this property will be displayed. The path to the loaded skin directory and the skinImgDir are prepended to this filename to form the full URL. If you need to set this state dynamically, use MenuItem.dynamicIcon instead.
		 * Flags: IR, Group: menuIcons */
		 disabledIcon?: string; // Flags=IR
		/** (Read only) Arbitrary UI component that should appear in this MenuItem. See ListGridRecord.embeddedComponent for an overview and options for controlling placement. When embeddedComponent is used in a MenuItem certain default behaviors apply: MenuItem.autoDismiss defaults to false and clicks on embeddedComponents are not bubbled to the menuItem - if an interaction with an embeddedComponent is expected to dismiss the menu, custom code should call menu.Menu.hide','hide or Menu.hideAllMenus','hideAllMenus as appropriate, before proceeding the default behavior for MenuItem.embeddedComponentPosition is "expand". the component is placed over the title and key fields by default - use MenuItem.embeddedComponentFields to override rollOver styling is disabled by default (as though ListGridRecord.showRollOver were set to false) 
		 * Flags: IR, Group: menuBasic */
		 embeddedComponent?: Canvas; // Flags=IR
		/** (Read only) See ListGridRecord.embeddedComponentFields. Default for a MenuItem is to cover the title and key fields, leaving the icon and submenu fields visible.
		 * Flags: IR, Group: menuBasic */
		 embeddedComponentFields?: Array<string> /* Array of String */; // Flags=IR
		/** (Read only) See ListGridRecord.embeddedComponentPosition, except that when used in a menuItem, default behavior is EmbeddedPosition "expand".
		 * Flags: IR, Group: menuBasic */
		 embeddedComponentPosition?: EmbeddedPosition; // Flags=IR
		/** (Read only) Affects the visual style and interactivity of the menu item. If set to false, the menu item will not respond to mouse rollovers or clicks. If you need to set this state dynamically, use MenuItem.enableIf instead.
		 * Flags: IR, Group: menuBasic */
		 enabled?: boolean /* Boolean */; // Flags=IR
		/** (Read only) Criteria to be evaluated to determine whether this MenuItem should be disabled. Re-evaluated each time the menu is shown. A basic criteria uses textMatchStyle:"exact". When specified in componentXML','Component XML this property allows xmlCriteriaShorthand','shorthand formats for defining criteria.
		 * Flags: IR, Group: ruleCriteria */
		 enableWhen?: AdvancedCriteria; // Flags=IR
		/** (Read only) If false, no submenus will be fetched for this MenuItem. This can be set globally via Menu.fetchSubmenus.
		 * Flags: IR */
		 fetchSubmenus?: boolean /* Boolean */; // Flags=IR
		/** (Read only) The filename for this item's custom icon. If both this property and MenuItem.checked are both specified, only the icon specified by this property will be displayed. The path to the loaded skin directory and the skinImgDir are prepended to this filename to form the full URL. If this item is disabled, and MenuItem.disabledIcon is set, then that icon will be used instead. If you need to set this state dynamically, use MenuItem.dynamicIcon instead.
		 * Flags: IR, Group: menuIcons */
		 icon?: string; // Flags=IR
		/** (Read only) The height applied to this item's icon. The default of 16 can be changed for all MenuItems by overriding Menu.iconHeight.
		 * Flags: IR, Group: menuIcons */
		 iconHeight?: number; // Flags=IR
		/** (Read only) The width applied to this item's icon. The default of 16 can be changed for all MenuItems by overriding Menu.iconWidth.
		 * Flags: IR, Group: menuIcons */
		 iconWidth?: number; // Flags=IR
		/** (Read only) When set to true, this menu item shows a horizontal separator instead of the MenuItem.title text. Typically specified as the only property of a menu item, since the separator will not respond to mouse events.
		 * Flags: IR, Group: menuBasic */
		 isSeparator?: boolean /* Boolean */; // Flags=IR
		/** (Read only) Shortcut key(s) to fire the menu item action. Each key can be defined as a KeyIdentifier. To apply multiple shortcut keys to this item, set this property to an array of such key identifiers.
		 * Flags: IR, Group: menuKeys */
		 keys?: Array<string> /* KeyIdentifier | Array of KeyIdentifer */; // Flags=IR
		/** (Read only) A string to display in the shortcut-key column for this item. If not specified, the first KeyName value in MenuItem.keys will be used by default.
		 * Flags: IR, Group: menuKeys */
		 keyTitle?: string; // Flags=IR
		/** (Read only) When used in an AdaptiveMenu, should this MenuItem show only it's MenuItem.icon','icon when displayed inline?
		 * Flags: IR */
		 showIconOnlyInline?: boolean; // Flags=IR
		/** (Read only) A reference to another menu, to display as a submenu when the mouse cursor hovers over this menu item.
		 * Flags: IR, Group: menuBasic */
		...
		/**  
		 *  Contains the condition that will check or uncheck the current menuItem. The handler must be specified as a function or string of script. Return false to uncheck the menuItem or true to check it If you don't need to set this state dynamically, use MenuItem.checked instead. May be defined as a stringMethods','stringMethod. 
		...
		/**  
		 *  Executed when this menu item is clicked by the user. The click handler must be specified as a function or string of script. Return false to suppress the Menu.itemClick handler if specified.
		 * @param {Canvas} target - for a menu shown as a context menu, the Canvas the menu was shown on. Otherwise the Menu instance of which this MenuItem is a member.
	...
	export interface HeaderSpan  {
		...
		/** (Read only) List of fields that this header spans. Fields should be identified by their value for ListGridField.name. Developers may define multiple levels of header-spans by specifying HeaderSpan.spans however a span cannot be specified with both fields and spans.
		 * Flags: IR, Group: headerSpan */
		...
		/** (Read only) Optional title for the headerSpan button for this headerSpan. If specified this will be displayed in the headerSpan button instead of HeaderSpan.title. Set to an empty string to suppress the title in the header button entirely.
		 * Flags: IR, Group: headerSpan */
		...
		/** (Read only) Name for this headerSpan, for use in APIs like ListGrid.setHeaderSpanTitle. Name is optional, but if specified, must be unique for this ListGrid (but not globally unique) as well as a valid JavaScript identifier, as specified by ECMA-262 Section 7.6 (the String.isValidID function can be used to test whether a name is a valid JavaScript identifier).
		 * Flags: IR, Group: headerSpan */
		 name?: string /* identifier */; // Flags=IR
		/** (Read only) This property allows developer to "nest" header spans, grouping fields together by multiple layers of granularity. For example a group of fields could be nested within two layers of header spans as follows: { title:"Europe", spans:[ {title:"France", fields:["Paris", "Lyon"]}, {title:"UK", fields:["London", "Glasgow"]}, {title:"Spain", fields:["Barcelona"]} ] } Note that a span definition can not include both spans and HeaderSpan.fields','fields.
		 * Flags: IR, Group: headerSpan */
		 spans?: Array<HeaderSpan> /* Array of HeaderSpan */; // Flags=IR
		/** (Read only) A title for this headerSpan, to display in the headerSpan button for this headerSpan and in other contexts such as the ListGrid.canPickFields','menu for picking visible fields. Note: if you want to use HTML tags to affect the display of the header, you should do so via HeaderSpan.headerTitle instead so that other places where the title appears in the UI are not affected. Refer to discussion at ListGridField.title.
		 * Flags: IR, Group: headerSpan */
		 title?: string /* String */; // Flags=IR
		/** (Read only) Vertical alignment of the title of this headerSpan. Defaults to listGrid.headerSpanVAlign if unset.
		 * Flags: IR, Group: headerSpan */
		 valign?: VerticalAlignment; // Flags=IR
		/** (Read only) Should the span title wrap if there is not enough space horizontally to accommodate it. If unset, default behavior is derived from ListGrid.wrapHeaderSpanTitles. (This is a soft-wrap - if set the title will wrap at word boundaries.)
		 * Flags: IR */
	...
	export interface ListGridRecord  extends Record {
		...
		/** (Read only) Default property name denoting whether this record can be edited. Property name may be modified for the grid via ListGrid.recordEditProperty.
		...
		/** (Read only) When set to false, other records cannot be dropped on (i.e., inserted via drag and drop) immediately before this record.
		 * Flags: IR */
		 canAcceptDrop?: boolean; // Flags=IR
		/** (Read only) When set to false, this record cannot be dragged. If canDrag is false for any record in the current selection, none of the records will be draggable.
		 * Flags: IR */
		 canDrag?: boolean; // Flags=IR
		/** (Read only) Default property name denoting whether this record can be expanded. Property name may be modified for the grid via ListGrid.canExpandRecordProperty.
		 * Flags: IR, Group: expansionField */
		 canExpand?: boolean; // Flags=IR
		/** (Read only) Default property name denoting whether this record can be selected. Property name may be modified for the grid via ListGrid.recordCanSelectProperty.
		 * Flags: IR */
		...
		/** (Read only) Set to false to disable rollover for this individual record when ListGrid.showRollOver is true. Note this property can be renamed to prevent collision with data members - see ListGrid.recordShowRollOverProperty.
		 * Flags: IR, Group: appearance */
	...
	export interface Criterion  {
		/** (Read only) For a criterion with an operator that acts on other criteria (eg "and", "or"), a list of sub-criteria that are grouped together by the operator.
		...
		/** (Read only) Name of the field in each Record that this criterion applies to. Not applicable for a criterion with Criterion.criteria','sub-criteria. Can be specified as a dataPath to allow matching nested objects. Use '/' as delimiters for dataPath. See DataPath','dataPath for more information.
		 * Flags: IR, Group: advancedFilter */
		 fieldName?: string /* String */; // Flags=IR
	...
	export interface ListGridField  {
		...
		/** (Read only) Per-field setting for ListGrid.allowFilterOperators. Can be used to enable the filter operators UI for a particular field if the ListGrid-level setting is not enabled, or to disable filter operators for a particular field if the ListGrid-level setting is enabled.
		 * Flags: IR */
		 allowFilterOperators?: boolean /* Boolean */; // Flags=IR
		/** (Read only) Per-field setting for ListGrid.alwaysShowOperatorIcon. Can be used to force a particular field to always show it's ListGrid.operatorIcon','operatorIcon, even if it has no filter-value, or is using the default ListGridField.filterOperator','search operator.
		 * Flags: IR */
		...
		/** (Read only) Whether this field should be automatically frozen when other fields are frozen. When true, the field will be automatically frozen to the extreme of the grid. The automatically generated ListGrid.checkboxField','checkbox, ListGrid.expansionField','expansion and ListGrid.rowNumberField','rowNumber fields are examples of fields that specify autoFreeze: true. You can control the position of this field in the array of frozen fields by providing a ListGridField.getAutoFreezePosition implementation.
		...
		/** (Read only) Can formula be edited from header context menu? Setting attribute to false prevents editing. A null or true value allows editing. Has no effect when ListGrid.canAddFormulaFields is false.
		 * Flags: IR */
		 canEditFormula?: boolean /* Boolean */; // Flags=IR
		/** (Read only) Can summary be edited from header context menu? Setting attribute to false prevents editing. A null or true value allows editing. Has no effect when ListGrid.canAddSummaryFields is false.
		 * Flags: IR */
		 canEditSummary?: boolean /* Boolean */; // Flags=IR
		/** (Read only) Dictates whether the data in this field be exported. Explicitly set this to false to prevent exporting. Has no effect if the underlying DataSourceField.canExport','dataSourceField is explicitly set to canExport: false.
		 * Flags: IR */
		...
		/** (Read only) Whether this field should display freezing/unfreezing options in its header context menu. See frozenFields.
		 * Flags: IR, Group: frozenFields */
		...
		/** (Read only) Whether this field can be reordered using the mouse. If unset, the default behavior is governed by ListGrid.canReorderFields. Note that setting this property to false will lock this field from being moved - that is, the user is prevented from moving this field directly by dragging with the mouse, or by dropping another field onto this field. However, unless this field is at the extreme edge of the grid, or all fields between it and the extreme edge of the grid are also canReorder: false, (ie, if there are unlocked fields on either side of this field), then it is possible that this locked field may be reordered automatically, as a result of the user dragging one unlocked field onto another unlocked field.
		 * Flags: IR */
		...
		/** (Read only) For a field of type:"icon" only, set the icon that appears in body cells. Unless setting ListGridField.icon','field.icon, setting field.cellIcon will not show an icon in the header. To change this property after fields have been passed to ListGrid.setFields, use ListGrid.setFieldCellIcon.
		 * Flags: IR */
		 cellIcon?: string /* SCImgURL */; // Flags=IR
		/** (Read only, Advanced) dataPath for this field. This property allows the grid to display details of nested data structures in a flat list of columns.
		...
		/** (Advanced) If this ListGrid is showing a filter row, this property can be used to apply a default value to show in the filter editor for this field.
		 * Flags: IRWA, Group: filterEditor */
		 defaultFilterValue?: any; // Flags=IRWA
		/** (Read only) Default groupingMode used when the user does not specify a mode or grouping is triggered programmatically and ListGridField.groupingMode','field.groupingMode is unset. See ListGridField.groupingModes','field.groupingModes.
		 * Flags: IR, Group: grouping */
		 defaultGroupingMode?: string /* identifier */; // Flags=IR
		/** (Read only, Advanced) If this field is editable, this property will be passed to editors displayed for cells within this field as FormItem.defaultIconSrc.
		...
		/** (Read only) Optional "default width" for this field. If set, this value will be returned by the ListGrid.getDefaultFieldWidth method, and used as the autoFit size for the field's content.
		 * Flags: IR, Group: autoFitFields */
		...
		/** (Advanced) If a ListGridField.displayField is set, should this field show record values from the displayField? If ListGridField.displayField is specified, and there is no separate ListGridField.optionDataSource, by default we will show display-field values from the same record. Setting this property to false would disable this behavior. Alternatively, if there is a ListGridField.optionDataSource (and ListGridField.autoFetchDisplayMap is false), the displayField would be ignored for the field and the underlying ListGridField.name','record[fieldName] value would be displayed to the user. This property may be set to true to override this behavior and pick up values from the displayField for display in this field even when there is an optionDataSource set. Note that this property has no effect on fields with an explicitly specified valueMap, or with an optionDataSource where ListGridField.autoFetchDisplayMap is true.
		 * Flags: IRWA */
		 displayValueFromRecord?: boolean; // Flags=IRWA
		/** (Read only) Shortcut to configure a FormItem.formula for the ListGridField.editorType','editor used when this field is ListGrid.canEdit','edited.
		 * Flags: IR */
		 editorFormula?: UserFormula; // Flags=IR
		/** (Read only, Advanced) If this field is editable, this property will be passed to editors displayed for cells within this field as FormItem.iconHeight. If this property unset, the iconHeight property from the editor can be picked up from ListGridField.iconHeight instead.
		 * Flags: IRA, Group: editing */
		 editorIconHeight?: number; // Flags=IRA
		/** (Read only, Advanced) If this field is editable, this property will be passed to editors displayed for cells within this field as FormItem.iconWidth. If this property unset, the iconWidth property from the editor can be picked up from ListGridField.iconWidth instead.
		 * Flags: IRA, Group: editing */
		 editorIconWidth?: number; // Flags=IRA
		/** (Advanced) When some cell in this field is being edited, this property can be used to apply an explicit FormItem.imageURLPrefix to the editor in question. This can be used to modify the valueIcons within the editor. If unset, but ListGridField.imageURLPrefix is specified, that will be used instead.
		 * Flags: IRWA, Group: editing */
		 editorImageURLPrefix?: string; // Flags=IRWA
		/** (Advanced) When some cell in this field is being edited, this property can be used to apply an explicit FormItem.imageURLSuffix to the editor in question. This can be used to modify the valueIcons within the editor. If unset, but ListGridField.imageURLPrefix is specified, that will be used instead.
		 * Flags: IRWA, Group: editing */
		 editorImageURLSuffix?: string; // Flags=IRWA
		/** (Advanced) Properties to apply the the form item created to edit this field. (Only used if this field is editable). For example, if you have a field "shoeSize" with DataSourceField.editorType set to "SpinnerItem" in order to use a SpinnerItem as your field editor, and you want to pass the SpinnerItem.step property to the created SpinnerItem: fields : [ { name:"shoeSize", editorType:"SpinnerItem", editorProperties : { step:0.5 } }, ... other fields ... ] 
		 * Flags: IRWA, Group: editing */
		 editorProperties?: FormItemProps /* FormItem properties */; // Flags=IRWA
		/** (Read only) Shortcut to configure a FormItem.textFormula for the ListGridField.editorType','editor used when this field is ListGrid.canEdit','edited.
		 * Flags: IR */
		 editorTextFormula?: UserSummary; // Flags=IR
		/** (Advanced) Name of form item class to use for the form item created to edit this field. (Only used if this field is editable). Note: If this is not specified, the edit-form item type may be derived from the editorType property, typically inherited from datasource fields, or from the type of the field (showing the appropriate form item for the data-type). See the editing overview for more on editing ListGrid fields.
		 * Flags: IRWA, Group: editing */
...
// Skipping editorValueMap because it has a getter
		...
		/** (Read only) Dictates whether the data in this field should be exported raw by ListGrid.exportClientData','exportClientData(). If set to true for a field, the values in the field-formatters will not be executed for data in this field. Decreases the time taken for large exports.
		 * Flags: IR */
		 exportRawValues?: boolean /* Boolean */; // Flags=IR
		/** (Advanced) If this ListGrid is showing a filter row (ListGrid.showFilterEditor','showFilterEditor:true), this property can be used to specify properties for the appropriate filter form item.
		 * Flags: IRWA, Group: filterEditor */
		 filterEditorProperties?: FormItemProps /* FormItem properties */; // Flags=IRWA
		/** (Advanced) If this ListGrid is showing a filter row, this property can be used to specify the form item class to use for the filter form item associated with this field (Only used if this field is not canFilter:false). Note: If this is not specified, the edit-form item type may be derived from the 'editorType' property, typically inherited from datasource fields, or from the 'type' of the field (showing the appropriate form item for the data-type).
		 * Flags: IRWA, Group: filterEditor */
		...
		/** (Read only) Optional title for the header button for this field. If specified this will be displayed in the header button instead of ListGridField.title or ListGridField.name. Set to an empty string to suppress the title in the header button entirely.
		 * Flags: IR, Group: appearance */
		 ...
		 * Flags: IRW, Group: appearance */
		 headerTitleStyle?: string /* CSSStyleName */; // Flags=IRW
		/** (Read only) Marks field as initially hidden. The user will still be able to show the field via a context menu. This may be suppressed by setting ListGridField.canHide to false, or by setting ListGrid.canPickFields to false to suppress the field-picker entirely. To mark a field as completely hidden (not shown to a user at all, in any component), set DataSourceField.hidden instead.
		 * Flags: IR, Group: appearance */
		 hidden?: boolean /* Boolean */; // Flags=IR
		/** (Read only) If set to true, custom HTML applied as part of hiliting will be applied after ListGrid.formatCellValue','formatting for each cell in this column. If false, hilite HTML will be applied before formatting. This attribute overrides ListGrid.hiliteHTMLAfterFormat as defined at the component level.
		 * Flags: IR */
		...
		/** (Read only) When ListGrid.hiliteIcons are present, where the hilite icon will be placed relative to the field value. See HiliteIconPosition. Overrides ListGrid.hiliteIconPosition.
		 * Flags: IR, Group: hiliting */
		...
		/** (Read only, Advanced) If this field is editable, this property will be passed to editors displayed for cells within this field as FormItem.iconVAlign.
		 * Flags: IRA, Group: editing */
		...
		/** (Advanced) If this field has type [ListGridFieldType] set to "image" and the URL for the image displayed is not absolute, the path of the URL will be relative to this string Alternatively, if this field displays any valueIcons, this prefix will be applied to the beginning of any ListGridField.valueIcons when determining the URL for the image.
		 * Flags: IRWA, Group: imageColumns */
		 imageURLPrefix?: string; // Flags=IRWA
		/** (Advanced) If any cells in this field are showing a value icon (see: ListGridField.valueIcons) or this is has ListGridFieldType set to "image", this the value of this property will be appended to the end of the URL for the icon displayed. Typical usage might be to append a file type such as ".gif" to the filename of the image. For editable fields, this property will also be passed through to any editors as FormItem.imageURLSuffix.
		 * Flags: IRWA, Group: imageColumns */
		...
		/** (Advanced) If this field has type [ListGridFieldType] set to "link", setting this property will apply a standard prefix to the link URL for cells in this field.
		 * Flags: IRWA */
		 linkURLPrefix?: string; // Flags=IRWA
		/** (Advanced) If this field has type [ListGridFieldType] set to "link", setting this property will apply a standard suffix to the link URL for cells in this field.
		 * Flags: IRWA */
		...
		/** (Read only) If this field has an optionDataSource specified and ListGridField.autoFetchDisplayMap','autoFetchDisplayMap is set, this attribute provides a way to customize the dataSource request issued to fetch the display map from the option dataSource. This provides, among other capabilities, a way to trigger the server to return summary records.
		 * Flags: IR, Group: display_values */
		 optionFilterContext?: DSRequestProps /* DSRequest Properties */; // Flags=IR
		/** (Read only) If this field has an optionDataSource specified and ListGridField.autoFetchDisplayMap','autoFetchDisplayMap is set, this attribute provides a way to customize the DSRequest.operationId passed to dataSource.fetchData() when retrieving the display map from the option dataSource.
		 * Flags: IR, Group: display_values */
		 optionOperationId?: string; // Flags=IR
		/** (Read only) For fields with an ListGridField.optionDataSource, where ListGridField.autoFetchDisplayMap is true, this property will govern the textMatchStyle attribute of the DSRequest parameter passed to DataSource.fetchData when retrieving the remote data set to be used as a basis for this field's valueMap.
		 * Flags: IR, Group: display_values */
		 optionTextMatchStyle?: TextMatchStyle; // Flags=IR
		/** (Read only) Only applies to fields of type "summary". This attribute is set on a summary field, when calculating the summary value from some record, the summary function will only be passed the fields before this summary field. This may be useful for displaying running totals across a record. Note that this feature would typically be used with ListGrid.canReorderFields','canReorderFields:false
		 * Flags: IR */
		 partialSummary?: boolean; // Flags=IR
		/** (Read only) Causes a tooltip hover to appear on the header generated for this field (effectively sets Canvas.prompt for the header).
		 * Flags: IR */
		 prompt?: string /* HTMLString */; // Flags=IR
		/** (Read only) Only applies to fields of type "summary". Fields of this type will display a calculated value based on the other field values within the current record. This attribute specifies how the summary field value will be calculated. See RecordSummaryFunction for valid options. A subset of the ListGrid's fields will be passed to the RecordSummaryFunction. Which fields to include is determined based on ListGridField.includeInRecordSummary If ListGrid.showGridSummary or ListGrid.showGroupSummary is true, this field's value in the summary row[s] will still be calculated by calling this method. In this case, the record object passed in will contain summary values for each field. If custom handling is required for this case, it may be detected by checking the record object's ListGridRecord.isGroupSummary and ListGridRecord.isGridSummary attributes.
		 * Flags: IR */
		...
		/** (Read only) If ListGrid.showGridSummary is true, should this field show a summary value. If unset, this field will show a summary value in the summary row if an explicit ListGridField.summaryFunction is specified or if a SimpleType.getDefaultSummaryFunction','default summary function is defined for the specified field type.
		 * Flags: IR */
		 showGridSummary?: boolean /* Boolean */; // Flags=IR
		/** (Read only) If ListGrid.showGroupSummary is true, should this field show a summary value in a summary row when the grid is grouped? If unset, this field will show a summary value in the summary row if an explicit ListGridField.summaryFunction is specified or if a SimpleType.getDefaultSummaryFunction','default summary function is defined for the specified field type.
		 * Flags: IR */
		...
		/** (Advanced) When set to true and showHover is also true for the field, shows a widget hovering at the mouse point. A number of builtin modes are provided - see HoverMode. Also supported at the ListGrid.showHoverComponents','ListGrid-level.
		...
		/** (Read only) If ListGrid.showGridSummary or ListGrid.showGroupSummary is true and the ListGridField.summaryFunction is set to "title", this attribute may be set to a string to display in the group and/or grid summary. If unspecified the ListGridField.title will show up in the summary.
		 * Flags: IR */
		...
		/** (Read only) ListGrids picks a renderer for the view and edit mode of a field based on this attribute. See ListGridFieldType for a summary of how types are rendered.
		 * Flags: IR, Group: appearance */
		...
		// method:ListGridField.getEditorValueMap skipped because it has been excluded due to: Getter method return type does not match attribute type.
	...
	export interface TreeNode  {
		/** (Read only, Advanced) Governs whether dragged data (typically other treeNodes) may be dropped over this node. Only has an effect if this node is displayed in a TreeGrid where TreeGrid.canAcceptDroppedRecords, TreeGrid.canReorderRecords or TreeGrid.canReparentNodes is true.
		 * Flags: IRA */
		 canAcceptDrop?: boolean; // Flags=IRA
		/** (Read only, Advanced) Governs whether this node can be dragged. Only has an effect if this node is displayed in a TreeGrid where TreeGrid.canDragRecordsOut, TreeGrid.canReorderRecords or TreeGrid.canReparentNodes is true.
		 * Flags: IRA */
		...
		/** (Read only) Specifies the unique ID of this node. Required for trees with Tree.modelType "parent". With modelType:"parent", the unique ID of a node, together with the unique ID of its parent (see TreeNode.parentId) is used by Tree.linkNodes to link a list of nodes into a tree. Note: the name of this property can be changed by setting Tree.idField.
		 * Flags: IR */
		 id?: string | number /* String or Number */; // Flags=IR
		/** (Read only) Set to true or a string that is not equal to (ignoring case) "false" to explicitly mark this node as a folder. See Tree.isFolder for a full description of how the Tree determines whether a node is a folder or not. Note: the name of this property can be changed by setting Tree.isFolderProperty.
		 * Flags: IR */
		 isFolder?: boolean | string /* Boolean or String */; // Flags=IR
		/** (Read only) Provides a name for the node that is unique among its immediate siblings, thus allowing a unique path to be used to identify the node, similar to a file system. See Tree.getPath. If the nameProperty is not set on a given node, the TreeNode.id will be used instead. If this is also missing, Tree.getName and Tree.getPath will auto-generate a unique name for you. Thus names are not required, but if the dataset you are using already has usable names for each node, using them can make APIs such as Tree.find more useful. Alternatively, if your dataset has unique ids consider providing those as TreeNode.id. If a value provided for the nameProperty of a node (e.g. node.name) is not a string, it will be converted to a string by the Tree via ""+value. This property is also used as the default title for the node (see Tree.getTitle) if TreeNode.title is not specified. Note: the name of this property can be changed by setting Tree.nameProperty.
		 * Flags: IR */
		 name?: string /* String */; // Flags=IR
		/** (Read only) For trees with modelType:"parent", this property specifies the unique ID of this node's parent node. The unique ID of a node, together with the unique ID of its parent is used by Tree.linkNodes to link a list of nodes into a tree. Note: the name of this property can be changed by setting Tree.parentIdField.
		 * Flags: IR */
		 parentId?: string | number /* String or Number */; // Flags=IR
		/** (Advanced) For folder nodes showing custom icons (set via TreeNode.icon), this property allows the developer to specify on a per-node basis whether a drop state icon should be displayed when the user drop-hovers over this folder. Set node.showDropIcon to true to show the drop state icon, or false to suppress this. If not specified, this behavior is determined by TreeGrid.showCustomIconDrop for this node. You can change the name of this property by setting TreeGrid.customIconDropProperty.
		 * Flags: IRWA, Group: treeIcons */
		 showDropIcon?: boolean /* Boolean */; // Flags=IRWA
		/** (Advanced) For folder nodes showing custom icons (set via TreeNode.icon), this property allows the developer to specify on a per-node basis whether an open state icon should be displayed when the folder is open. Set node.showOpenIcon to true to show the open state icons, or false to suppress this. If not specified, this behavior is determined by TreeGrid.showCustomIconOpen for this node. You can change the name of this property by setting TreeGrid.customIconOpenProperty.
		 * Flags: IRWA, Group: treeIcons */
		 showOpenIcon?: boolean /* Boolean */; // Flags=IRWA
		/** (Read only) The title of the node as it should appear next to the node icon in the Tree. If left unset, the value of TreeNode.name is used by default. See the description in Tree.getTitle for full details. Note: the name of this property can be changed by setting Tree.titleProperty.
		 * Flags: IR */
		 title?: string /* HTML */; // Flags=IR
	...
	export interface AdvancedCriteria  extends Criterion {
		/** (Advanced) When set to true, causes filtering using this criteria object to follow SQL99 behavior for dealing with NULL values. See DataSource.strictSQLFiltering','this discussion for more detail.
	...
	export interface DetailViewerField  {
		 ...
		 * Flags: IRW, Group: hiliting */
		 canHilite?: boolean; // Flags=IRW
// Skipping cellStyle because it has a getter
		 ...
		 * Flags: IRW, Group: appearance */
		 decimalPrecision?: number; // Flags=IRW
		/** (Read only) If displayField is defined for the field then the DetailViewer will display the displayField attribute of records instead of the attribute given by the DetailViewerField.name of the field.
		 * Flags: IR */
		 displayField?: string /* String */; // Flags=IR
		/** (Read only) The value to display for a cell whose value is null or the empty string after applying formatCellValue and valueMap (if any). This is the field-specific attribute. You may also set the emptyCellValue at the viewer level to define the emptyCellValue for all empty fields in the viewer.
		 * Flags: IR, Group: appearance */
		 emptyCellValue?: string /* HTMLString */; // Flags=IR
		/** (Read only) By default HTML values in DetailViewer cells will be interpreted by the browser. Setting this flag to true will causes HTML characters to be escaped, meaning the raw value of the field (for example "&lt;b&gt;AAA&lt;/b&gt;") is displayed to the user rather than the interpreted HTML (for example "AAA")
		 * Flags: IR */
		 escapeHTML?: boolean; // Flags=IR
		/** (Read only) FormatString used during exports for numeric or date formatting. See DataSourceField.exportFormat.
		 * Flags: IR, Group: exportFormatting */
		 exportFormat?: string /* FormatString */; // Flags=IR
		/** (Read only) Dictates whether the data in this field should be exported raw by DetailViewer.exportClientData','exportClientData(). If set to true for a field, the values in the field-formatters will not be executed for data in this field.
		 * Flags: IR */
		 exportRawValues?: boolean /* Boolean */; // Flags=IR
		/** (Read only) FormatString for numeric or date formatting. See DataSourceField.format.
		 * Flags: IR, Group: exportFormatting */
		 format?: string /* FormatString */; // Flags=IR
		/** (Read only) For DetailViewerField.type: "separator", this attribute specifies the height of the separator.
		 * Flags: IR */
		 ...
		 * Flags: IRW, Group: hiliting */
		 hiliteIconLeftPadding?: number; // Flags=IRW
		/** (Read only) When DetailViewer.hiliteIcons are present, where the hilite icon will be placed relative to the field value. See HiliteIconPosition. Overrides DetailViewer.hiliteIconPosition
		 * Flags: IR, Group: hiliting */
		 hiliteIconPosition?: HiliteIconPosition; // Flags=IR
		...
		/** (Advanced) If this field has type set to "image" and the URL for the image displayed is not absolute, the path of the URL will be relative to this string
		 * Flags: IRWA, Group: imageColumns */
		 imageURLPrefix?: string; // Flags=IRWA
		...
		/** (Read only) Indicates this field's values come from another, related DataSource. The individual field will inherit settings such as DetailViewerField.type','field.type and DetailViewerField.title','field.title from the related DataSource just like fields from the primary DataSource.
		 * Flags: IR */
		 ...
		 * Flags: IRW */
		 linkTextProperty?: string; // Flags=IRW
		/** (Advanced) If this field has type set to "link", setting this property will apply a standard prefix to the link URL when displaying values of this field.
		 * Flags: IRWA */
		 linkURLPrefix?: string; // Flags=IRWA
		/** (Advanced) If this field has type set to "link", setting this property will apply a standard suffix to the link URL when displaying values of this field.
		 * Flags: IRWA */
		 linkURLSuffix?: string; // Flags=IRWA
		/** (Read only) Name property used to identify the field, and determines which attribute from records will be displayed in this field. Must be unique within the DetailViewer as well as a valid JavaScript identifier, as specified by ECMA-262 Section 7.6 (the String.isValidID function can be used to test whether a name is a valid JavaScript identifier). The attribute of the records to display in this field may also be set by DetailViewerField.displayField.
		 * Flags: IR */
		 name?: string /* String */; // Flags=IR
	...
	export interface Tab  {
		...
		/** (Read only) Criteria to be evaluated to determine whether this Tab should be enabled. Re-evaluated whenever data in the Canvas.ruleScope','tab.ruleScope changes. It works the same as Canvas.enableWhen
		 * Flags: IR, Group: ruleCriteria */
		 enableWhen?: AdvancedCriteria; // Flags=IR
		...
		/** (Read only) Space to leave around the pane within this Tab. If specified, this property takes precedence over TabSet.paneMargin
		 * Flags: IR */
	...
	export interface UserSummary  {
		/** (Read only) Map from the all-capital letter names used as summary variables in the UserSummary String to the fieldNames these variables should represent in the context where the summary is evaluated. When used in ListGridField.userSummary context, field names are evaluated against the grid record. When used in FormItem.textFormula or ListGridField.editorTextFormula this property is not used for summary mapping. Instead, field names are evaluated directly against the current Canvas.ruleScope','rule context.
		 * Flags: IR */
	...
	export interface PaletteNode  {
		/** (Read only) If set to false, indicates that this node cannot be EditProxy.useCopyPasteShortcuts','copy &amp; pasted, including disallowing calls to EditContext.makePaletteNode for EditNode','EditNodes created from this PaletteNode','PaletteNode.
		 * Flags: IR */
		 canDuplicate?: boolean /* Boolean */; // Flags=IR
		/** (Read only) Defaults for the component to be created from this palette. For example, if PaletteNode.type is "ListGrid", properties valid to pass to Class.create','ListGrid.create(). Note that event handlers or method overrides cannot be configured as defaults, as they cannot be serialized or restored. Instead, create a subclass that implements the desired behaviors, and use that subclass as PaletteNode.type. 
		 * Flags: IR */
		 defaults?: Properties; // Flags=IR
	...
	export interface FormItemIcon  {
		/** (Advanced) Base CSS style. If set, as the component changes state and/or is focused, suffixes will be added to the base style. Possible suffixes include "Over" if the user mouses over the icon and FormItemIcon.showOver','this.showOver is true, "Disabled" if the icon is disabled, and "Focused". In addition, if FormItemIcon.showRTL','showRTL is enabled, then an "RTL" suffix will be added.
		 * Flags: IRWA */
		 ...
		 * Flags: IRW, Group: appearance */
		 disabled?: boolean /* Boolean */; // Flags=IRW
		/** (Advanced) If FormItem.canEdit is set to false, should this icon be disabled. If unset this is determined by FormItem.disableIconsOnReadOnly. Note that if FormItemIcon.neverDisable is set to true, the icons will be rendered enabled regardless of this setting and whether the item is editable.
